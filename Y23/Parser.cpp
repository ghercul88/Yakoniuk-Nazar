...
namespace Parser {
    ...
    unsigned int IdIdentification(struct id idTable[], struct Token TokenTable[], unsigned int tokenCount) {
        ...
        while (TokenTable[i++].type != Data && TokenTable[i].type != EndBlock);
    ...
    void statement() {
        ...
        case Get:     inputStatement();   break;
        case Put:    outputStatement();  break;
    ...
    unsigned int LabelIdentification(struct id labelTable[], struct Token TokenTable[], unsigned int tokenCount) {
        ...
        while (TokenTable[start++].type != MaimProgram);
        ...
        for (int currLabel = 0; currLabel < labelNum; ++currLabel) {
            for (int currId = 0; currId < idNum; ++currId) {
                if (!strcmp(labelTable[currLabel].name, idTable[currId].name))
                    printf("\n Semantic error: label %s is already defined as identifier\n", labelTable[currLabel].name);
                    fprintf(errorFile, "\n Semantic error: label %s is already defined as identifier\n", labelTable[currLabel].name);
                    exit(1);
            }
        }
